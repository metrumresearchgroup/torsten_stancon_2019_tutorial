...
\label{sec:org01e477a}
\begin{frame}[label={sec:org031dc1a}]{}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org747bc36}]{Outline}
\begin{block}{Day 1}
\begin{itemize}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org9fe25c1}]{Logistics}
...
\begin{frame}[label={sec:org9c0f77b}]{Logistics}
Workshop package
\begin{itemize}
...
\label{sec:org3c8ba62}

\begin{frame}[label={sec:orge1f3096}]{Preliminary question}
\begin{itemize}
...
\begin{frame}[label={sec:org0304f62}]{Modeling framework}
...
\begin{frame}[label={sec:org07dde16}]{Inference}
\begin{itemize}
...
\end{enumerate}
\end{frame}
\begin{frame}[label={sec:org017490b}]{Diagnosing the inference algorithm}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org705aed7}]{Example: fitting a linear model}
Likelihood:
\begin{align*}
...
\label{sec:org4fea8bb}
\begin{frame}[label={sec:orgd80c375}]{What is the effect of a treatment on a patient?}
\begin{itemize}
...
\begin{frame}[label={sec:org09e2212}]{Example: Two compartment model}
\begin{center}
  \includegraphics[width=4in]{TwoCptNice.png}
\end{center}
\end{frame}

\begin{frame}[label={sec:org3c6b5af}]{Two compartment model}
\begin{align*}
...
\begin{frame}[label={sec:org2a1fb57}]{Example 2: Bone mineral density model from \cite{Peterson:2012}}
\begin{center}
  \includegraphics[width = 3in]{RiggsBoneModel.jpg}
\end{center}
\end{frame}

\begin{frame}[label={sec:orgcb9b2d1}]{Two compartment model}
...
\begin{frame}[label={sec:org746250f}]{The event schedule}
An event can be:
\begin{itemize}
...
\begin{frame}[label={sec:org0bf084f}]{Drug concentration in a patient's blood}
\begin{center}
  \includegraphics[width=3.5in]{multiple_doses.png}
\end{center}
\end{frame}

\begin{frame}[label={sec:org3e591a4}]{The event schedule}
\begin{itemize}
...
\begin{frame}[label={sec:org8d36dcc}]{Torsten functions}
...
\end{itemize}
\end{frame}
\begin{frame}[fragile,label={sec:org0b74d32}]{Torsten functions}
...
\begin{frame}[label={sec:orgcdc07d5}]{Example}
...
\begin{frame}[label={sec:orgc74a91f}]{Example}
\begin{block}{Model}
\begin{itemize}
...
\end{block}
\end{frame}
\begin{frame}[fragile,label={sec:orgb1f1f1c}]{Example}
 \begin{block}{Prior}
\begin{minted}[breaklines=true,fontsize=\footnotesize,breakanywhere=true]{stan}
...
\begin{frame}[label={sec:org86ce635}]{Resources}
\begin{itemize}
...
\label{sec:org65a59af}
\begin{frame}[label={sec:orgeed1fb8}]{Arsenal of tools}
\begin{center}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org5c0e3ac}]{Matrix exponential}
...
$$ y(t) = e^{tK} y_0 $$
\end{frame}
\begin{frame}[label={sec:orgf24b24b}]{Matrix Exponential}
$$ e^{tK} = \sum_{n=0}^{\infty} \dfrac{(tK)^n}{n!} = I + tK + \frac{(tK)^2}{2} + \frac{(tK)^3}{3!} + ... $$
\end{frame}
\begin{frame}[label={sec:org6625787}]{Matrix Exponential}
...
\[ K = \begin{bmatrix}
  ...
  \]
\end{frame}
\begin{frame}[fragile,label={sec:org78e591a}]{Linear ODE solver in Torsten}
 \begin{minted}[breaklines=true,fontsize=\footnotesize,breakanywhere=true]{stan}
...
\end{minted}
\end{frame}
\begin{frame}[fragile,label={sec:org6bdaca8}]{Numerical integrator}
 \begin{minted}[breaklines=true,fontsize=\footnotesize,breakanywhere=true]{stan}
...
\end{itemize}
\end{frame}
\begin{frame}[fragile,label={sec:org28ed1ae}]{System function}
 \begin{minted}[breaklines=true,fontsize=\footnotesize,breakanywhere=true]{stan}
functions {
...
\label{sec:org334c8df}
\begin{frame}[label={sec:orgd959fd0}]{Data pooled into groups}
\begin{itemize}
...
\begin{frame}[fragile,label={sec:org6ee13bf}]{Data pooled into groups}
 \begin{itemize}
\item medical measurements are grouped by patients
\begin{itemize}
...
\begin{frame}[label={sec:orgadb6c69}]{Hierarchical model}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org430cc9a}]{Hierarchical model}
$$\theta = (\theta_1, ..., \theta_L) \sim p(\theta | \theta_\mathrm{pop}) $$
$$y = (y_1, ..., y_N) \sim p(y | \theta, x) $$
\begin{center}
...
\begin{frame}[label={sec:org99fd2d8}]{Example 3: Hierarchical two compartment model}
Likelihood function:
\begin{align*}
...
\begin{frame}[label={sec:org7b63857}]{Exercise 6: Write, fit, and diagnose a hierarchical two}
...
\end{itemize}
\end{frame}
\begin{frame}[fragile,label={sec:org6561f81}]{Divergent transitions}
 \begin{itemize}
\item \emph{Do you get any warning messages?}
...
\begin{frame}[label={sec:orgc54a16f}]{Divergent transitions}
...
\begin{frame}[label={sec:org190b7a4}]{Divergent transitons}
$$ \alpha_i \sim \mathrm{Normal}(\mu, \sigma) $$
...
\begin{frame}[label={sec:org356a25d}]{Divergent transitons}
\begin{center}
  \includegraphics[width=5cm]{pairs_baseball2.png}
\end{center}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org80a3795}]{Reparameterization}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org86e27ec}]{Reparameterization}
Then
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org6076483}]{Reparameterization}
...
\begin{frame}[label={sec:orga8d9a6d}]{Reparameterization}
\begin{center}
  \includegraphics[width=9cm]{twoCptPairs1}
\end{center}
\end{frame}

\begin{frame}[label={sec:org8fb01d2}]{Reparameterization}
...
\begin{frame}[label={sec:orgff4ed46}]{Reparameterization}
Need:
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org751c9e4}]{Individual predictions}
\begin{center}
  \includegraphics[width = 10cm]{PredictionPatient.png}
\end{center}
\end{frame}

\begin{frame}[label={sec:org1feeca5}]{Population predictions}
\begin{center}
  \includegraphics[width = 10cm]{PredictionPopulation.png}
\end{center}
\end{frame}

\begin{frame}[label={sec:orgac335d0}]{Further reading}
...
\label{sec:org935311a}

\begin{frame}[fragile,label={sec:orgac2f64b}]{ODE group integrators}
 \begin{center}
\begin{tabular}{ll}
...
\begin{frame}[fragile,label={sec:org9d36bdc}]{PMX population solvers}
 \begin{center}
\begin{tabular}{ll}
Single ODE system & ODE group\\
\hline
...
\begin{frame}[fragile,label={sec:orgbb3e88e}]{PMX population solvers}
 \begin{minted}[breaklines=true,fontsize=\footnotesize,breakanywhere=true]{stan}
matrix
...
\end{columns}
\end{frame}
\begin{frame}[label={sec:org8e8bf08}]{Exercise}
...
\begin{frame}[label={sec:org3c0d309}]{Exercise}
...
\begin{frame}[label={sec:org7c0c315}]{ODE system of F-K model}
...
\end{itemize}
\end{frame}
\begin{frame}[fragile,label={sec:org63f2b1e}]{Coupled PK-PD system}
...
\begin{frame}[fragile,label={sec:orgc7bb1f2}]{Coupled PK-PD system}
 \begin{minted}[breaklines=true,fontsize=\footnotesize,breakanywhere=true]{stan}
real[] pmx_solve_twocpt_rk45(reduced_ODE_system, int nOde, real[] time, real[] amt, real[] rate, real[] ii, int[] evid, int[] cmt, real[] addl, int[] ss, real[] theta, real[] biovar, real[] tlag, real rel_tol, real abs_tol, real max_step)
\end{minted}
\begin{itemize}
...
\begin{frame}[fragile,label={sec:org0af7a53}]{Reduced system}
 \begin{minted}[breaklines=true,fontsize=\footnotesize,breakanywhere=true]{stan}
...
\end{frame}
\begin{frame}[label={sec:org484206e}]{Exercise}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:orgc2533be}]{Exercise}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:orga68c97b}]{Exercise}
\begin{center}
  \includegraphics[width=7cm]{FKModelPlots002.pdf}
\end{center}
\end{frame}

\begin{frame}[label={sec:org65e89c8}]{Exercise}
\begin{center}
  \includegraphics[width=7cm]{FKPairs}
\end{center}
\end{frame}

\begin{frame}[label={sec:orge2e0617}]{Exercise}
\begin{center}
  \includegraphics[width=7cm]{FKModelPlots006.pdf}
\end{center}
\end{frame}

\begin{frame}[label={sec:org29ddcbc}]{Reference}
