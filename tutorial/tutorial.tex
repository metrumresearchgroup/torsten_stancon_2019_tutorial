...
\label{sec:org0ec7894}
\begin{frame}[label={sec:org9d3916f}]{}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org7d0f031}]{Outline}
\begin{block}{Day 1}
\begin{itemize}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org0f747d0}]{Logistics}
...
\begin{frame}[label={sec:org3449079}]{Logistics}
Workshop package
\begin{itemize}
...
\label{sec:org0a308cc}

\begin{frame}[label={sec:orgf05c1df}]{Preliminary question}
\begin{itemize}
...
\begin{frame}[label={sec:org0403179}]{Modeling framework}
...
\begin{frame}[label={sec:orga3c804c}]{Inference}
\begin{itemize}
...
\end{enumerate}
\end{frame}
\begin{frame}[label={sec:org06b8deb}]{Diagnosing the inference algorithm}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org2ee9d2b}]{Example: fitting a linear model}
Likelihood:
\begin{align*}
...
\label{sec:org5454063}
\begin{frame}[label={sec:org1f6756d}]{What is the effect of a treatment on a patient?}
\begin{itemize}
...
\begin{frame}[label={sec:org18da075}]{Example: Two compartment model}
\begin{center}
  \includegraphics[width=4in]{TwoCptNice.png}
\end{center}
\end{frame}

\begin{frame}[label={sec:org69f73cc}]{Two compartment model}
\begin{align*}
...
\begin{frame}[label={sec:orgac7a859}]{Example 2: Bone mineral density model from \cite{Peterson:2012}}
\begin{center}
  \includegraphics[width = 3in]{RiggsBoneModel.jpg}
\end{center}
\end{frame}

\begin{frame}[label={sec:org77317bc}]{Two compartment model}
...
\begin{frame}[label={sec:org7ba58b9}]{The event schedule}
An event can be:
\begin{itemize}
...
\begin{frame}[label={sec:org718cf67}]{Drug concentration in a patient's blood}
\begin{center}
  \includegraphics[width=3.5in]{multiple_doses.png}
\end{center}
\end{frame}

\begin{frame}[label={sec:orgc15d14c}]{The event schedule}
\begin{itemize}
...
\begin{frame}[label={sec:orgace7a7c}]{Torsten functions}
...
\end{itemize}
\end{frame}
\begin{frame}[fragile,label={sec:org38372c9}]{Torsten functions}
...
\begin{frame}[label={sec:org1348690}]{Example}
...
\begin{frame}[label={sec:org060a12e}]{Example}
\begin{block}{Model}
\begin{itemize}
...
\end{block}
\end{frame}
\begin{frame}[fragile,label={sec:org5f6c713}]{Example}
 \begin{block}{Prior}
\begin{minted}[breaklines=true,fontsize=\footnotesize,breakanywhere=true]{stan}
...
\begin{frame}[label={sec:org8f3ea0d}]{Resources}
\begin{itemize}
...
\label{sec:orgdd149a3}
\begin{frame}[label={sec:org25e8ef3}]{Arsenal of tools}
\begin{center}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:orgc3f061e}]{Matrix exponential}
...
$$ y(t) = e^{tK} y_0 $$
\end{frame}
\begin{frame}[label={sec:org37869d7}]{Matrix Exponential}
$$ e^{tK} = \sum_{n=0}^{\infty} \dfrac{(tK)^n}{n!} = I + tK + \frac{(tK)^2}{2} + \frac{(tK)^3}{3!} + ... $$
\end{frame}
\begin{frame}[label={sec:org9a3e658}]{Matrix Exponential}
...
\[ K = \begin{bmatrix}
  ...
  \]
\end{frame}
\begin{frame}[fragile,label={sec:org9f42567}]{Linear ODE solver in Torsten}
 \begin{minted}[breaklines=true,fontsize=\footnotesize,breakanywhere=true]{stan}
...
\end{minted}
\end{frame}
\begin{frame}[fragile,label={sec:org890a825}]{Numerical integrator}
 \begin{minted}[breaklines=true,fontsize=\footnotesize,breakanywhere=true]{stan}
...
\end{itemize}
\end{frame}
\begin{frame}[fragile,label={sec:org063336c}]{System function}
 \begin{minted}[breaklines=true,fontsize=\footnotesize,breakanywhere=true]{stan}
functions {
...
\begin{frame}[fragile,label={sec:orgfe8a9c1}]{Numerical integrators}
 \begin{center}
\begin{tabular}{lll}
...
\begin{frame}[fragile,label={sec:org1eb05b0}]{Exercise}
 \begin{itemize}
...
\begin{frame}[fragile,label={sec:org8aa8a5d}]{Exercise}
...
\begin{frame}[fragile,label={sec:orgea6268a}]{Exercise}
 How to build \& run?
...
\label{sec:orgd952418}
\begin{frame}[label={sec:org4fe1ee2}]{Data pooled into groups}
\begin{itemize}
...
\begin{frame}[fragile,label={sec:org8b75977}]{Data pooled into groups}
 \begin{itemize}
\item medical measurements are grouped by patients
\begin{itemize}
...
\begin{frame}[label={sec:org937b8ec}]{Hierarchical model}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:orgedbc523}]{Hierarchical model}
$$\theta = (\theta_1, ..., \theta_L) \sim p(\theta | \theta_\mathrm{pop}) $$
$$y = (y_1, ..., y_N) \sim p(y | \theta, x) $$
\begin{center}
...
\begin{frame}[label={sec:org6ef4bed}]{Example 3: Hierarchical two compartment model}
Likelihood function:
\begin{align*}
...
\begin{frame}[label={sec:org73d2640}]{Exercise 6: Write, fit, and diagnose a hierarchical two}
...
\end{itemize}
\end{frame}
\begin{frame}[fragile,label={sec:orga1d8cb1}]{Divergent transitions}
 \begin{itemize}
\item \emph{Do you get any warning messages?}
...
\begin{frame}[label={sec:org68ee3b1}]{Divergent transitions}
...
\begin{frame}[label={sec:org836ee7f}]{Divergent transitons}
$$ \alpha_i \sim \mathrm{Normal}(\mu, \sigma) $$
...
\begin{frame}[label={sec:orga2d4809}]{Divergent transitons}
\begin{center}
  \includegraphics[width=5cm]{pairs_baseball2.png}
\end{center}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org11c0724}]{Reparameterization}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org9a3b099}]{Reparameterization}
Then
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:orgce8e08f}]{Reparameterization}
...
\begin{frame}[label={sec:org51f4b62}]{Reparameterization}
\begin{center}
  \includegraphics[width=9cm]{twoCptPairs1}
\end{center}
\end{frame}

\begin{frame}[label={sec:orga9a0ac5}]{Reparameterization}
...
\begin{frame}[label={sec:orgd8567ba}]{Reparameterization}
Need:
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org0b4fc9f}]{Individual predictions}
\begin{center}
  \includegraphics[width = 10cm]{PredictionPatient.png}
\end{center}
\end{frame}

\begin{frame}[label={sec:orgcedecc2}]{Population predictions}
\begin{center}
  \includegraphics[width = 10cm]{PredictionPopulation.png}
\end{center}
\end{frame}

\begin{frame}[label={sec:orgdaadcdc}]{Further reading}
...
\label{sec:orgc44cbd2}

\begin{frame}[fragile,label={sec:org3ad625c}]{ODE group integrators}
 \begin{center}
\begin{tabular}{ll}
...
\begin{frame}[fragile,label={sec:org8547d32}]{ODE group integrators}
 \begin{columns}
\begin{column}{0.45\columnwidth}
\begin{block}{Single ODE system}
\begin{minted}[breaklines=true,fontsize=\footnotesize,breakanywhere=true]{stan}
...
\begin{frame}[fragile,label={sec:org87797d2}]{PMX population solvers}
 \begin{center}
\begin{tabular}{ll}
Single ODE system & ODE group\\
\hline
...
\begin{frame}[fragile,label={sec:org4974391}]{PMX population solvers}
 \begin{minted}[breaklines=true,fontsize=\footnotesize,breakanywhere=true]{stan}
matrix
...
\end{columns}
\end{frame}
\begin{frame}[label={sec:org36a123b}]{Exercise}
...
\begin{frame}[label={sec:org397a417}]{Exercise}
...
\begin{frame}[label={sec:orgebd8b9c}]{ODE system of F-K model}
...
\end{itemize}
\end{frame}
\begin{frame}[fragile,label={sec:orgb926992}]{Coupled PK-PD system}
...
\begin{frame}[fragile,label={sec:orgddc6ab6}]{Coupled PK-PD system}
 \begin{minted}[breaklines=true,fontsize=\footnotesize,breakanywhere=true]{stan}
real[] pmx_solve_twocpt_rk45(reduced_ODE_system, int nOde, real[] time, real[] amt, real[] rate, real[] ii, int[] evid, int[] cmt, real[] addl, int[] ss, real[] theta, real[] biovar, real[] tlag, real rel_tol, real abs_tol, real max_step)
\end{minted}
\begin{itemize}
...
\begin{frame}[fragile,label={sec:orgc991d49}]{Reduced system}
 \begin{minted}[breaklines=true,fontsize=\footnotesize,breakanywhere=true]{stan}
...
\end{frame}
\begin{frame}[label={sec:org2861acf}]{Exercise}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org90b196d}]{Exercise}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:orgd6d9bb1}]{Exercise}
\begin{center}
  \includegraphics[width=7cm]{FKModelPlots002.pdf}
\end{center}
\end{frame}

\begin{frame}[label={sec:orgb42f072}]{Exercise}
\begin{center}
  \includegraphics[width=7cm]{FKPairs}
\end{center}
\end{frame}

\begin{frame}[label={sec:orgcc87ec1}]{Exercise}
\begin{center}
