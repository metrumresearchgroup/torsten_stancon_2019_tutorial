...
generated quantities {
  // simulate new data for the observed patients
  real obs_pred[ntot] = lognormal_rng(log(x3), sigma);

  // simulate data for new (simulated) patients
  real obs_new_pred[ntot];
  real y0_1_new[nsub];
  real y0_new[3];
  real x_new[ntot, 3];
  real x3_new[ntot];
  ...
  for (i in 1:nsub) {
    y0_1_new[i] = lognormal_rng(y0_mu, 0.5);
    y0_new[1] = y0_1_new[1];
    x_new[i1[i]:i2[i], ] = pmx_integrate_ode_bdf(reaction, y0, t0,
                                ts[i1[i]:i2[i]], theta, xr, xi,
                                1e-4, 1e-8, 10000);
  }
  x3_new = x_new[, 3];
  obs_new_pred = lognormal_rng(log(x3_new), sigma);
}
